let studentsTableBody=document.querySelector("#students-table tbody"),getStudentsButton=document.getElementById("get-students-btn"),studentForm=document.getElementById("add-student-form");function getStudents(){fetch("http://localhost:3000/students").then(t=>t.json()).then(renderStudents).catch(()=>{})}function renderStudents(t){studentsTableBody.innerHTML=makeStudentsMarkup(t)}let makeStudentsMarkup=t=>t.map(t=>`
    ...
    `).join("");function addStudent(t){return fetch("http://localhost:3000/students",{method:"POST",body:JSON.stringify(t),headers:{"Content-Type":"application/json; charset=UTF-8"}})}function updateStudent(t){fetch(`http://localhost:3000/students/${t}`).then(t=>t.json()).then(({name:e,age:d,course:n,skills:u,email:s,isEnrolled:a})=>{studentForm.name.value=e,studentForm.age.value=d,studentForm.course.value=n,studentForm.skills.value=u.join(", "),studentForm.email.value=s,studentForm.isEnrolled.checked=a,editingId=t})}function deleteStudent(t){return fetch(`http://localhost:3000/students/${t}`,{method:"DELETE"})}getStudentsButton.addEventListener("click",getStudents),studentForm.addEventListener("submit",t=>{t.preventDefault(),addStudent({name:form.name.value.trim(),age:Number(form.age.value),course:form.course.value.trim(),skill:form.skill.value.trim(),email:form.email.value.trim(),isEnrolled:form.isEnrolled.checked}).then(()=>{studentForm.reset(),getStudents()}).catch(()=>{})}),studentsTableBody.addEventListener("click",t=>{let e=t.target.dataset.id;t.target.classList.contains("edit-btn")&&updateStudent(e)}),studentsTableBody.addEventListener("click",t=>{t.target.classList.contains("delete-btn")&&deleteStudent(t.target.dataset.id).then(()=>getStudents()).catch(()=>{})});
//# sourceMappingURL=hm__18.0b76ec8a.js.map
